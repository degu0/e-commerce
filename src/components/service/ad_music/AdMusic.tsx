...
export function AdMusic () {
    ...
    Object.keys(timeLeft).forEach((interval) => {
        ...
        timerComponents.push(
            <div className="bg-white rounded-full flex flex-col justify-center items-center w-14 h-14 lg:w-16 lg:h-16 xl:w-16 xl:h-16" key={interval}>
                <p className="font-semibold">{timeLeft[interval]}</p>
                <p className="text-xs xl:text-ms">{interval}</p>
            </div>
        );
    });
    ...
    return (
        <div className="bg-black w-[92%] 2xl:w-full h-[500px] rounded flex">
            <div className="h-full w-1/2 flex flex-col gap-10 justify-center items-start ml-5 lg:ml-16 text-white">
                ...
                <h1 className="text-white text-4xl lg:text-5xl">Enhance Your <br /> Music Experience</h1>
                <div className="flex items-center gap-2 text-black">
                    {timerComponents.length ? timerComponents : <span>Time's up!</span>}
                </div>
                <button className="bg-green-500 text-white py-3 px-8 rounded">Buy Now!</button>
            </div>
